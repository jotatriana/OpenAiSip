...
> **Precedence fix (found via live-call testing):** simply adding `report_new_issue` to the tool list wasn't enough — the model kept following the older, heavily-reinforced `## CRITICAL — No Billing Access` scripted deflection instead of calling the new tool when a caller raised billing mid-RESOLVE/WRAP_UP. `_base_instructions()`'s billing block, the `## Priority` section, and the RESOLVE/WRAP_UP phase instructions now all explicitly state that `report_new_issue` takes precedence over scripted redirects whenever it's in the current tool list. Verified with two follow-up live calls exercising both a billing and a sales mid-call topic change — see **Lessons Learned** below.
...
**Scripted instructions can silently out-compete newer tool-based instructions.** Adding a tool to `_tools_for_phase()`'s tool list is not enough to make the model actually use it — if an older, heavily-reinforced scripted phrase (e.g. the `## CRITICAL — No Billing Access` block, repeated verbatim in every phase via `_base_instructions()`) already covers the same trigger, the model follows the scripted phrase instead unless explicit precedence is stated. This is exactly what happened when `report_new_issue` (the RESOLVE/WRAP_UP → TRIAGE loopback tool) was first added: live-call testing showed the model kept using the billing-deflection script instead of calling the new tool, even though the tool was correctly present and wired end-to-end. Fix: state the precedence explicitly at every point of conflict — the billing block, the `## Priority` section, and the RESOLVE/WRAP_UP phase instructions all now say `report_new_issue` wins whenever it's in the current tool list. Don't rely on a tool's own description to win a fight against an existing scripted directive elsewhere in the prompt.

**Scenario tests can't catch tool-*selection* bugs.** `ScenarioHarness.call_tool(name, args)` invokes a named tool directly — it bypasses the model's decision of *whether* to call that tool at all, and in which of several competing ways to respond. A prompt change that alters which tool the model reaches for among competing options can pass the entire test suite while still being broken end-to-end, because no test ever asks the model to choose. Any change to cross-cutting/optional tool behavior needs at least one live call to verify before considering it done. To check what actually happened: query `call_transcripts` for the call's actual turns (`SELECT turn_index, role, phase, text FROM call_transcripts WHERE call_id=? ORDER BY turn_index`), not just the app log's raw `OpenAI event: X` stream, which logs every low-level realtime event (including per-character audio transcript deltas) and is too noisy to read directly — grep it for `Phase entered|Tool call:|Tool .* result:|Service category|WRAP_UP complete|Escalat` instead to get the high-level trace.

**Repository tests write to whatever real `DATABASE_URL` points at, with no per-test isolation or teardown.** Fine for per-account-scoped tables — fake test account_ids like `ACC-BA1234` never collide with a real caller's account. But it's a real correctness risk for global/shared tables: `products`/`promotions` test rows (literal `description="test"` placeholders) surfaced directly in a live caller's product catalog and promotions list, and `service_areas` tests used a random 3-digit numeric zip prefix that could have collided with and corrupted a real caller's `get_service_eligibility` result. When writing tests against global tables, avoid production-realistic-looking keys (or clean up rows after the test) — a fake `account_id` is naturally namespaced away from real callers, but a fake zip prefix or product ID is not.

**`python main.py` does not hot-reload.** After any code change, a running dev instance must be killed and restarted before re-testing against it, or it keeps serving stale code: `pgrep -f "^python main.py$"` to find the PID, `kill <pid>`, then relaunch.
